...
Image formats support: all formats supported by image-rs by default: AVIF, BMP, DDS, EXR, FF, GIF w/ animation support, HDR, ICO, JPEG, PNG w/ APNG animation support, PNM, QOI, TGA, TIFF, WEBP w/ animation support (see https://docs.rs/image/latest/image/codecs/index.html#supported-formats ); JXL (via jxl-oxide) and HEIC (via libheif-rs).
...
You can use Nix expression to build&install with Nix (naively via `nix-env -i -f default.nix`) or use the Makefile (deps: C and Rust toolchains, SDL3, libheif).
...
- some reasonable configurability
...
- (maybe) rewrite remaining part in Rust
